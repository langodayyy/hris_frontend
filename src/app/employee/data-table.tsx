...
export function DataTable<TData, TValue>({
  ...
  const [selectedPosition, setSelectedPosition] =
    useState<DepartmentPosition | null>(null);
    ...
    const fetchData = async () => {
      try {
        const resDepPos = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/department-position`,
          {
            headers: {
        ...
        if (!resDepPos.ok)
          throw new Error("Failed to fetch Department & Position");
        const dataDepPos: DepartmentPosition[] = await resDepPos.json();

        setDepPosData(dataDepPos);

        const uniqPositions = Array.from(
          new Set(dataDepPos.map((item) => item.Position))
        );
        ...
        const uniqueDepartments = Array.from(
          new Map(
            dataDepPos.map((item) => [
              item.id_department,
              { id: item.id_department, name: item.Department },
            ])
          ).values()
        );
  ...
  const handleImportButton = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/employees/preview-csv`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${Cookies.get("token")}`,
  ...
  return (
    < >
    <div className=""  id="table-employee">
      ...
      <div className="flex items-center py-4 gap-[10px]">
        ...
        <div className="w-fit" >
          ...
          >
            <DropdownMenuTrigger asChild id="filter-employee">
              <Button
                variant="outline"
                icon={
                  <svg
        ...
        <div className="w-fit">
          <Dialog>
            <DialogTrigger asChild id="export-employee">
        ...
        <div className="w-fit">
          <Dialog>
            <DialogTrigger asChild id="import-employee">
              <Button className="w-[100px]" variant="outline" size="lg">
                <svg
                  width="15"
                  height="15"
                ...
                >
                  <g clipPath="url(#clip0_462_2152)">
                    <path
                      d="M13.75 9.99963V13.1246C13.75 13.2904 13.6842 13.4494 13.5669 13.5666C13.4497 13.6838 13.2908 13.7496 13.125 13.7496H1.875C1.70924 13.7496 1.55027 13.6838 1.43306 13.5666C1.31585 13.4494 1.25 13.2904 1.25 13.1246V9.99963H0V13.1246C0 13.6219 0.197544 14.0988 0.549175 14.4505C0.900805 14.8021 1.37772 14.9996 1.875 14.9996H13.125C13.6223 14.9996 14.0992 14.8021 14.4508 14.4505C14.8025 14.0988 15 13.6219 15 13.1246V9.99963H13.75Z"
                      fill="currentColor"
                    />
                    <path
                      d="M7.47924 -0.00014548C7.23311 -0.000823982 6.98926 0.0470776 6.76168 0.140814C6.53409 0.23455 6.32725 0.372278 6.15299 0.546105L3.70361 2.99548L4.58736 3.87923L6.85861 1.6086L6.87486 11.8749H8.12486L8.10861 1.61735L10.3705 3.87923L11.2542 2.99548L8.80486 0.546105C8.63072 0.372297 8.42398 0.234576 8.1965 0.140838C7.96902 0.0471004 7.72527 -0.000809992 7.47924 -0.00014548Z"
                      fill="currentColor"
                    />
                  </g>
                  <defs>
