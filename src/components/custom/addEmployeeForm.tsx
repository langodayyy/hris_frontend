...
const employeeSchema = z.object({
    ...
    education: z.enum(['SD', 'SMP', 'SMA', 'D3', 'S1', 'S2', 'S3'], { required_error: 'Education is required' }),
    blood_type: z.enum(['A', 'B', 'AB', 'O', 'Unknown'], { required_error: 'Blood type is required' }),
    ...
    religion: z.enum(['Islam', 'Protestant', 'Catholic', 'Hindu', 'Buddha', 'Confucianism', 'Other'], { required_error: 'Religion is required' }),
    contract_type: z.enum(['Permanent', 'Contract', 'Internship'], { required_error: 'Contract type is required' }),
    ...
    phone_number: z.string().min(10, { message: 'Phone number is invalid' }),
    email: z.string().email({ message: 'Invalid email address' }),
    bank_code: z.string().min(1, { message: 'Bank is required' }),
    account_number: z.string().min(1, { message: 'Account number is required' }),
    salary: z.preprocess(
        (val) => (val === "" ? undefined : Number(val)),
        z.number({ required_error: "Salary is required" }).min(0, { message: "Salary cannot be negative" })
    ),
