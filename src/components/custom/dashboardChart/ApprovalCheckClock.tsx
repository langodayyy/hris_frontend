...
const chartDataPie = chartDataPieFromDB.map((item) => {
  const key = item.status_type.toLowerCase().replace(/\s+/g, "_");
  return {
    ...item,
    fill: `var(--color-${key})`,
  };
...
const chartPieConfig = Object.fromEntries(
  chartDataPie.map((item, index) => {
    ...
    return [key, { color, label: item.status_type }];
  })
...
const dataSummary = chartDataPie.reduce((summary, item) => {
  ...
  const key = item.status_type.toLowerCase().replace(/\s+/g, "_"); // e.g., "pending_approval"
